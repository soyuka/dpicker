...
export function App (sources) {
  ...
  const max = CycleDPicker({name: 'max', model: dpicker.max, max: DPicker._dateAdapter.addYears(dpicker.max, 1), format: DEFAULT_FORMAT, order: DEFAULT_ORDER})
  ...
  .map((configArray) => {
    ...
    for (let i = 0; i < formLength; i++) {
      ...
      if (param.key === 'siblingMonthDayClick' && dpicker._data.siblingMonthDayClick !== param.value) {
        dpicker._data[param.key] = param.value
        dpicker.redraw()
        continue
      }
