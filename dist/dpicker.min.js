...
function DPicker(t) {
    ...
    var a = this._getContainer(t), i = a.container, r = a.attributes;
    ...
    for (var u in s)
        void 0 !== n[u] && (s[u] = n[u]);
    this._events = this._loadEvents(), this._loadModules(r, n), this._createMethods(s, r), void 0 !== r.value && '' !== r.value || (this._data.empty = !0), this._setData('model', [
        r.value,
        n.model,
        moment()
    ...
    }), this.initialize(), this._mount(i), i.id = this._container, i.addEventListener('keydown', this._events.keyDown);
    var l = i.querySelector('input');
...
DPicker.prototype._setData = function (t, e) {
    for (var n = arguments.length > 2 && void 0 !== arguments[2] && arguments[2], a = 0; a < e.length; a++)
        if (void 0 !== e[a] && '' !== e[a]) {
            if (n === !1) {
                this._data[t] = e[a];
                break;
            }
            if (e[a] instanceof moment && e[a].isValid()) {
                this._data[t] = e[a];
                break;
            }
            this._data[t] = moment();
            var i = moment(e[a], this._data.format, !0);
            if (i.isValid()) {
                this._data[t] = i;
                break;
            }
    ...
    for (var e in this._modules.render)
        t[e] = this._modules.render[e](this._events, this._data);
    return t.days = this.renderDays(this._events, this._data), this._data.order.filter(function (e) {
        return t[e];
    }).map(function (e) {
        return t[e];
    });
}, DPicker.prototype._mount = function (t) {
    this._tree = this.getTree(), t.appendChild(this._tree);
}, DPicker.prototype._decorate = function (t, e, n) {
    var a = this;
    return function () {
        for (var i = a._modules[t][n], r = 0; r < i.length; r++)
            if (i[r].apply(a, arguments) === !1)
                return !1;
        return e.apply(a, arguments);
    };
}, DPicker.prototype._loadModules = function (t, e) {
    this._modules = {
        calls: {},
        events: {},
        render: {}
    ...
    for (var i in DPicker.modules) {
        for (var r = DPicker.modules[i], o = 0; o < n.length; o++) {
            var d = n[o];
            for (var s in r[d])
                if (d !== n[0] || void 0 !== this._events[s]) {
                    var u = r[d][s];
                    void 0 === this._modules[d][s] ? this._modules[d][s] = [u] : this._modules[d][s].push(u);
                } else
                    this._events[s] = r[d][s].bind(this);
        }
        if (r.render)
            for (var l in r.render) {
                if (l in this._modules.render)
                    throw new ReferenceError('Can not override a render method');
                this._modules.render[l] = r.render[l];
            }
        if (r.properties)
            for (var h in r.properties)
                if (!this._data[h]) {
                    var m = r.properties[h], c = 'function' == typeof m.attribute ? m.attribute(t) : t[m.attribute];
                    this._createGetSet(h), this._setData(h, [
                        c,
                        e[h],
                        m['default']
                    ], !!m.isMoment);
    ...
    for (var p = 0; p < n.length; p++)
        for (var f = n[p], v = 0 === p ? Object.keys(this._events) : a, _ = 0; _ < v.length; _++) {
            var y = v[_];
            if (void 0 !== this._modules[f][y])
                if (0 === p) {
                    var g = this._events[y];
                    this._events[y] = this._decorate(f, g, y);
                } else {
                    var C = this[y];
                    this[y] = this._decorate(f, C, y);
                }
        }
    ...
    return {
        ...
        inputBlur: function (e) {
            if (t.display !== !1) {
                ...
                isElementInContainer(n.parentNode, t._container) || (t.display = !1, t.onChange({
                    modelChanged: !1,
                    name: 'inputBlur',
                    event: e
                }));
        ...
        inputFocus: function (e) {
            t.display = !0, e.target && e.target.select && e.target.select(), t.onChange({
                modelChanged: !1,
                name: 'inputFocus',
                event: e
            });
        ...
        keyDown: function (e) {
            if (t._data.hideOnEnter) {
                ...
                13 !== n && 27 !== n || (document.getElementById(t.inputId).blur(), t.display = !1, t.onChange({
                    modelChanged: !1,
                    name: 'keyDown',
                    event: e
                }));
    ...
    var a;
    return a = document.createElement('input'), a.setAttribute('id', '' + String(e.inputId)), a.setAttribute('value', '' + String(e.empty === !0 ? '' : e.model.format(e.format))), a.setAttribute('type', 'text'), a.setAttribute('min', '' + String(e.min.format(e.format))), a.setAttribute('max', '' + String(e.max.format(e.format))), a.setAttribute('format', '' + String(e.format)), a.onchange = t.inputChange, a.onfocus = t.inputFocus, a.setAttribute('name', '' + String(e.inputName)), a.setAttribute('aria-invalid', '' + String(e.valid === !1)), a.setAttribute('aria-haspopup', 'aria-haspopup'), a.setAttribute('class', '' + String(e.valid === !1 ? 'dpicker-invalid' : '')), a;
}, DPicker.prototype.renderContainer = function (t, e, n) {
    var a;
    return a = document.createElement('div'), a.setAttribute('class', 'dpicker'), _appendChild(a, [n]), a;
}, DPicker.prototype.render = function (t, e, n) {
    var a;
    return a = document.createElement('div'), a.setAttribute('aria-hidden', '' + String(e.display === !1)), a.setAttribute('class', 'dpicker-container ' + String(e.display === !0 ? 'dpicker-visible' : 'dpicker-invisible')), _appendChild(a, [
        '\n    ',
        n,
        '\n    '
    ]), a;
}, DPicker.prototype.renderYears = function (t, e, n) {
    for (var a, i = e.model.year(), r = e.max.year() + 1, o = e.min.year(), d = []; --r >= o;)
        if (r === i) {
            var s;
    ...
    return a = document.createElement('select'), a.onchange = t.yearChange, a.setAttribute('name', 'dpicker-year'), a.setAttribute('aria-label', 'Year'), _appendChild(a, [d]), a;
}, DPicker.prototype.renderMonths = function (t, e, n) {
    var a, i = e.model.month(), r = e.model.year(), o = e.months, d = e.months.map(function (t, e) {
            return e;
        });
    if (e.max.year() === r) {
        var s = e.max.month();
    ...
    return a = document.createElement('select'), a.onchange = t.monthChange, a.setAttribute('name', 'dpicker-month'), a.setAttribute('aria-label', 'Month'), _appendChild(a, [
        '\n      ',
        d.map(function (t, e) {
            var n, a;
            return t === i ? (n = document.createElement('option'), n.setAttribute('value', '' + String(t)), n.setAttribute('selected', 'selected'), _appendChild(n, [o[t]]), n) : (a = document.createElement('option'), a.setAttribute('value', '' + String(t)), _appendChild(a, [o[t]]), a);
        }),
        '\n    '
    ]), a;
}, DPicker.prototype.renderDays = function (t, e, n) {
    var a, i, r = e.model.daysInMonth(), o = e.model.clone().subtract(1, 'months').daysInMonth(), d = e.firstDayOfWeek, s = +e.model.clone().date(1).format('e') - 1, u = e.model.date(), l = e.model.month(), h = e.model.year(), m = void 0, c = void 0, p = void 0, f = void 0, v = new Array(7);
    e.days.map(function (t, e) {
        v[e < d ? 6 - e : e - d] = t;
    }), e.model.isSame(e.min, 'month') && (m = e.min.date(), p = e.min.month()), e.model.isSame(e.max, 'month') && (c = e.max.date(), f = e.max.month());
    var _ = new Array(Math.ceil(0.1 + (s + r) / 7)).fill(0), y = void 0, g = void 0, C = !1, k = !1, D = !0, b = '';
    return i = document.createElement('table'), _appendChild(i, [
        '\n    ',
        (a = document.createElement('tr'), _appendChild(a, [v.map(function (t) {
                var e;
        ...
        _.map(function (n, a) {
            var i;
            return i = document.createElement('tr'), _appendChild(i, [new Array(7).fill(0).map(function (n, i) {
                    var d, m, c;
                    g = D, b = '', i <= s && 0 === a ? (y = o - (s - i), g = !1, C = !0) : i === s + 1 && 0 === a ? (C = !1, y = 1, g = !0) : (y === r && (y = 0, g = !1, D = !1, k = !0), y++);
                    var p = C ? l : k ? l + 2 : l + 1, f = moment(y + '-' + p + '-' + h, 'DD-MM-YYYY');
                    return g === !1 && e.siblingMonthDayClick === !0 && (g = !0), e.min && g && (g = f.isSameOrAfter(e.min, 'day')), e.max && g && (g = f.isSameOrBefore(e.max, 'day')), g === !0 && C === !1 && k === !1 && u === y && (b = 'dpicker-active'), d = document.createElement('td'), d.setAttribute('class', '' + String(g === !0 ? 'dpicker-active' : 'dpicker-inactive')), _appendChild(d, [
                        '\n          ',
                        g === !0 ? (m = document.createElement('button'), m.setAttribute('value', '' + String(y)), m.setAttribute('aria-label', 'Day ' + String(y)), m.setAttribute('aria-disabled', '' + String(g)), m.onclick = C === !1 && k === !1 ? t.dayClick : C === !0 ? t.previousMonthDayClick : t.nextMonthDayClick, m.setAttribute('type', 'button'), m.onkeydown = t.dayKeyDown, m.setAttribute('class', '' + String(b)), _appendChild(m, [
                            '\n                ',
                            y,
                            '\n              '
                        ]), m) : (c = document.createElement('span'), c.setAttribute('class', '' + String(b)), _appendChild(c, [y]), c),
                        '\n          '
                    ]), d;
                })]), i;
        }),
