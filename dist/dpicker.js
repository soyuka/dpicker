...
DPicker.prototype._setData = function (key, values) {
    ...
    for (var i = 0; i < values.length; i++) {
        ...
        var date = this.moment(values[i], this._data.format, true);
        if (date.isValid()) {
            this._data[key] = date;
            break;
        }
...
DPicker.prototype._createMethods = function (defaults, attributes) {
    for (var i in defaults) {
        ...
        this._setData(i, [
            attributes[i],
            defaults[i]
        ], defaults[i] instanceof this.moment);
    }
};
    ...
    return {
        ...
        inputChange: function inputChange(evt) {
            if (!evt.target.value) {
                ...
                var newModel = _this2.moment(evt.target.value, _this2._data.format, true);
    ...
    return _table = document.createElement('table'), _appendChild(_table, [
        ...
        rows.map(function (e, row) {
            ...
            return _tr2 = document.createElement('tr'), _appendChild(_tr2, [new Array(7).fill(0).map(function (e, col) {
                    ...
                    var currentDayModel = _this3.moment(day + '-' + dayMonth + '-' + currentYear, 'DD-MM-YYYY');
    ...
    'onChange': {
        set: function set(onChange) {
            ...
            this._onChange = function (dpickerEvent) {
                return !onChange ? false : onChange(_this4._data, dpickerEvent);
            };
        },
        get: function get() {
...
},{"moment":undefined,"nanomorph":1,"yo-yoify/lib/appendChild":4}]},{},[5])(5)
});