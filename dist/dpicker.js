...
DPicker.prototype.getRenderChild = function () {
    ...
    for (var _module in this._modulesRender) {
        for (var renderMethod in this._modulesRender[_module]) {
            children[renderMethod] = this._modulesRender[_module][renderMethod](this._events, this._data);
        }
    }
...
DPicker.prototype._loadModules = function loadModules(methods, attributes, options) {
    ...
    for (var moduleName in DPicker.modules) {
        var _module2 = DPicker.modules[moduleName];
        var _loop = function _loop(event) {
            if (!_this2._events[event]) {
                _this2._events[event] = _module2.events[event].bind(_this2);
                return 'continue';
            }
            if (!_this2._events[event + '-internal']) {
                _this2._events[event + '-internal'] = [_this2._events[event]];
                _this2._events[event] = function (evt) {
                    _this2._events[event + '-internal'].map(function (e) {
                        return e.bind(_this2)(evt);
                    });
                };
            }
            _this2._events[event + '-internal'].unshift(_module2.events[event]);
        };
        for (var event in _module2.events) {
            var _ret = _loop(event);
            if (_ret === 'continue')
                continue;
        }
        var _loop2 = function _loop2(call) {
            if (!DPicker.prototype.hasOwnProperty(call) || typeof _module2.calls[call] !== 'function') {
                return 'continue';
            }
            if (!_this2[call + '-internal']) {
                _this2[call + '-internal'] = [DPicker.prototype[call]];
                _this2[call] = function () {
                    for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
                        args[_key] = arguments[_key];
                    }
                    _this2[call + '-internal'].map(function (e) {
                        return e.apply(_this2, args);
                    });
                };
            }
            _this2[call + '-internal'].push(_module2.calls[call]);
        };
        for (var call in _module2.calls) {
            var _ret2 = _loop2(call);
            if (_ret2 === 'continue')
                continue;
        }
        if (_module2.render) {
            for (var i in _module2.render) {
                this._modulesRender[i] = _module2.render;
            }
        }
        if (_module2.properties) {
            for (var _i2 in _module2.properties) {
                if (this._data[_i2]) {
                    continue;
                }
                var prop = _module2.properties[_i2];
...
DPicker.prototype._loadEvents = function loadEvents() {
    ...
    return {
        hide: function hide(evt) {
            if (_this3._data.hideOnOutsideClick === false || _this3.display === false) {
                return;
            }
            var node = evt.target;
            if (isElementInContainer(node.parentNode, _this3._container)) {
                return;
            }
            _this3.display = false;
            _this3.onChange(false);
        },
        inputChange: function inputChange(evt) {
            if (!evt.target.value) {
                _this3._data.empty = true;
            } else {
                var newModel = moment(evt.target.value, _this3._data.format, true);
                if (_this3.isValid(newModel) === true) {
                    _this3._data.model = newModel;
                }
                _this3._data.empty = false;
            }
            _this3.redraw();
            _this3.onChange(true);
        },
        inputBlur: function inputBlur(evt) {
            if (_this3.display === false) {
                return;
            }
        ...
        yearChange: function yearChange(evt) {
            _this3._data.empty = false;
            _this3._data.model.year(evt.target.options[evt.target.selectedIndex].value);
            _this3.isValid(_this3._data.model);
            _this3.redraw();
            _this3.onChange(true);
        },
        monthChange: function monthChange(evt) {
            _this3._data.empty = false;
            _this3._data.model.month(evt.target.options[evt.target.selectedIndex].value);
            _this3.isValid(_this3._data.model);
            _this3.redraw();
            _this3.onChange(true);
        },
        dayClick: function dayClick(evt) {
            evt.preventDefault();
            evt.stopPropagation();
            _this3._data.model.date(evt.target.value);
            _this3._data.empty = false;
            if (_this3._data.hideOnDayClick) {
                _this3.display = false;
            }
            _this3.isValid(_this3._data.model);
            _this3.redraw();
        ...
        previousMonthDayClick: function previousMonthDayClick(evt) {
            if (!_this3._data.siblingMonthDayClick) {
                return;
            }
            evt.preventDefault();
            evt.stopPropagation();
            _this3._data.model.date(evt.target.value);
            _this3._data.model.subtract(1, 'month');
            _this3._data.empty = false;
            if (_this3._data.hideOnDayClick) {
                _this3.display = false;
            }
        ...
        nextMonthDayClick: function nextMonthDayClick(evt) {
            ...
            evt.stopPropagation();
            _this3._data.model.date(evt.target.value);
            _this3._data.model.add(1, 'month');
            _this3._data.empty = false;
            if (_this3._data.hideOnDayClick) {
                _this3.display = false;
            }
        ...
        keyDown: function keyDown(evt) {
            ...
            document.getElementById(_this3.inputId).blur();
            _this3.display = false;
            _this3.onChange(false);
        }
    };
...
Object.defineProperties(DPicker.prototype, {
    ...
    'onChange': {
        set: function set(onChange) {
            var _this4 = this;
            this._onChange = function (modelChanged) {
                return !onChange ? false : onChange(_this4._data, modelChanged);
            };
        },
        get: function get() {
