  ...
  return {
    ...
    inputChange: function(evt) {
      if (!evt.target.value) {
        ...
        var newModel = moment(evt.target.value, $__1._data.format, true);
        if ($__1.isValid(newModel)) {
          $__1._data.model = newModel;
        }
        $__1._data.empty = false;
      }
      $__1._minutesStep();
      $__1.onChange();
    },
...
DPicker.prototype.renderTime = injector(function renderTime(events, data, toRender) {
  ...
  if (data.model.isSame(data.min, 'day')) {
    ...
    var minHours = +data.meridiem ? data.min.format('h') : data.min.hours();
    hours = hours.filter(function(e) {
      return e >= minHours;
    });
  }
  if (data.model.isSame(data.max, 'day')) {
    var maxMinutes = data.max.minutes();
    minutes = minutes.filter(function(e) {
      return e <= maxMinutes;
    });
    var maxHours = +data.meridiem ? data.max.format('h') : data.max.hours();
    hours = hours.filter(function(e) {
      return e <= maxHours;
    });
  }
