...
DPicker.h = DPicker.prototype.h = function h(element, props, children) {
  ...
  for (var i$__21 = 0; i$__21 < children.length; i$__21++) {
    if (typeof children[i$__21] === 'string' || typeof children[i$__21] === 'number') {
      el.innerText = children[i$__21];
      break;
    }
    if (Array.isArray(children[i$__21])) {
      children[i$__21].map(function(node) {
        return frag.appendChild(node);
      });
    } else {
      frag.appendChild(children[i$__21]);
    }
  }
  el.appendChild(frag);
  return el;
